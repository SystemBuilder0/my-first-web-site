...
# "stream": F2로 시작/종료하되, 말하는 도중에 실시간으로 글자가 찍힘 (기본, 추천)
# "toggle": F2로 시작/종료. 종료를 눌러야 그때 한꺼번에 찍힘 (예전 방식)
...
# ---- MODE = "stream" 일 때만 사용되는 값들 (실시간 타이핑) ----
# 말하는 도중에 글자가 찍히게 하는 원리:
#   Whisper는 원래 "한 덩어리를 통째로" 인식하는 모델이라 진짜 스트리밍은 안 된다.
#   그래서 짧은 간격으로 "지금까지 녹음된 것 전체"를 계속 다시 인식하되,
#   방금 말한 부분은 뒤에 말이 더 붙으면 해석이 바뀔 수 있으므로 바로 찍지 않고,
#   "연속된 두 번의 인식 결과가 서로 일치하는 앞부분"만 확정해서 타이핑한다.
#   (LocalAgreement 방식) 이렇게 하면 이미 친 글자를 지웠다 다시 쓰는 일이 없다.

# 몇 초마다 다시 인식할지. 짧을수록 글자가 빨리 나오지만 GPU를 더 많이 쓴다.
STREAM_INTERVAL_SEC = 0.7

# 이보다 짧은 오디오는 인식을 시도하지 않는다(초).
STREAM_MIN_AUDIO_SEC = 0.4

# 다시 인식할 오디오 버퍼가 이보다 길어지면, 이미 확정된 앞부분을 잘라내서
# 매번 다시 인식하는 양이 계속 늘어나지 않게 한다(초).
STREAM_MAX_BUFFER_SEC = 24.0

# 앞부분을 잘라낼 때 문맥 유지를 위해 남겨둘 여유(초).
STREAM_KEEP_BACK_SEC = 1.5

# 실시간에는 속도가 중요하므로 1(가장 빠름)을 쓴다. 정확도를 원하면 5까지 올릴 수 있다.
STREAM_BEAM_SIZE = 1

# 방금 말한 부분(녹음 맨 끝)은 뒷말이 붙으면 해석이 바뀌므로, 뒤에 최소 이만큼(초)의
# 소리가 더 쌓이기 전에는 확정하지 않는다. 값을 키우면 더 정확해지지만 글자가
# 조금 더 늦게 나온다.
STREAM_RIGHT_CONTEXT_SEC = 0.3
